...
* News
- 2021-07-25: ~helm-bibtex-with-local-bibliography~ now also uses locally and globally defined bibliographies in org files.  These are bibliographies specified using the new ~#+BIBLIOGRAPHY:~ key word and those in the variable ~org-cite-global-bibliography~.
