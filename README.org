...
By default ~helm-bibtex~ uses the entire window to display the bibliography. This can be changed by setting the variable ~helm-bibtex-full-frame~ to ~nil~, which will split the window below instead.
