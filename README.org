...
Helm-bibtex’ main feature is efficient search in large bibliographies using powerful search expressions.  Actions that be performed on entries matching the search expression are:
- open the PDF, URL, or DOI,
- insert a citation, the BibTeX key, the BibTeX entry, a plain text reference,
- attach a PDF to an email,
- take notes,
- edit the BibTeX entry.
Many aspects of helm-bibtex can be configured to suit personal preferences.
...
The easiest way to install the latest version of helm-bibtex is through [[http://melpa.milkbox.net/#/][MELPA]].  Alternatively, put the file [[file:helm-bibtex.el]] in a directory included in your load-path and add the following line to your start-up file (typically ~init.el~):
...
At this point most people will be ready to go.  Skip to [[#usage]] below to see how to use helm-bibtex.
...
Start an email to your colleague and ~M-x helm-bibtex~.  Select your new publications and select “Add PDF to email.”  Then ~M-x helm-resume~ (the publications are still marked) and select “Insert BibTeX entry.”  Optionally insert more human readable references by ~M-x helm-resume~ and “Insert reference.”  Send email.  Done.  This takes less than 10 seconds.
